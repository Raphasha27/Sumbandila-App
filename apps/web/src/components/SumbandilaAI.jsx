...
const SumbandilaAI = () => {
  ...
  const [messages, setMessages] = useState([
    { role: 'ai', content: "Hello! I am Sumbandila AI, your personal Registry Sentinel. How can I help you verify an institution today?" }
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      if (lowerInput.includes('doctor') || lowerInput.includes('medical')) {
        aiResponse = "I can help you verify doctors via the HPCSA registry. Just type their name or registration number.";
      } else if (lowerInput.includes('school') || lowerInput.includes('college') || lowerInput.includes('education')) {
        aiResponse = "I have access to the Department of Higher Education (DHET) records. Which institution are you investigating?";
      } else if (lowerInput.includes('legal') || lowerInput.includes('lawyer')) {
        aiResponse = "I can verify legal practitioners through the Legal Practice Council. Do you have a name or practice number?";
      } else if (lowerInput.includes('risk') || lowerInput.includes('safe')) {
        aiResponse = "My latest Sentinel Alert indicates a 1.2% increase in bogus college registrations this month. Always look for the 'Sentinel Verified' badge.";
      }
              ...
              {messages.map((m, i) => (
                <motion.div 
                  ...
                  style={{ 
                    ...
                    maxWidth: '85%',
                    background: m.role === 'ai' ? 'white' : 'var(--primary-orange)',
                    ...
                    border: m.role === 'ai' ? '1px solid #F3F4F6' : 'none'
                  }}
                >
                  {m.content}
                </motion.div>
