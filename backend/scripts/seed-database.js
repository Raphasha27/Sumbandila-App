...
async function seedDatabase() {
    ...
    try {
        ...
        const { data: instData, error: instError } = await supabase
            .from('institutions')
            .upsert(institutions, { onConflict: 'institution_id' });

        if (instError) {
            console.error('❌ Error inserting institutions:', instError.message);
        } else {
            console.log(`✅ Successfully inserted ${institutions.length} institutions`);
        }

        // Seed professionals (doctors and lawyers combined)
        console.log('\n👨‍⚕️👨‍⚖️ Inserting professionals...');
        const professionals = [...doctors, ...lawyers];
        const { data: profData, error: profError } = await supabase
            .from('professionals')
            .upsert(professionals, { onConflict: 'professional_id' });

        if (profError) {
            console.error('❌ Error inserting professionals:', profError.message);
        } else {
            console.log(`✅ Successfully inserted ${professionals.length} professionals`);
        ...
        console.log(`   Total institutions: ${instCount}`);
        console.log(`   Total professionals: ${profCount}`);
